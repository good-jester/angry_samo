...
function resetpumpkin() {
  // Portal effect
  let disappearPortal = new PortalEffect(
    pumpkin.position.x,
    pumpkin.position.y
  );
...
function draw() {
  if (!gameStarted) {
    ...
    if (
      pumpkinReleased &&
      Math.abs(pumpkin.velocity.x) < 0.01 &&
      Math.abs(pumpkin.velocity.y) < 0.01
    ) {
      resetpumpkin();
    }
    ...
    if (!pumpkinHasCollided) {
      if (!pumpkinReleased) {
        angle = atan2(
          slingshot.origin.y - pumpkin.position.y,
          slingshot.origin.x - pumpkin.position.x
        );
      } else {
...
function propagateImpact(
  ...
  let bodiesInContact = Matter.Query.collides(
    body,
  ...
  for (let result of bodiesInContact) {
    ...
    let distanceFromOriginal = dist(
      originalCollisionPosition.x,
      originalCollisionPosition.y,
      otherBody.position.x,
      otherBody.position.y
    );

    // Reduce the impact magnitude based on the distance
    let reducedImpactMagnitude =
      impactMagnitude / (1 + distanceFromOriginal / 500); // Ajustar el divisor según sea necesario
    ...
    if (
      otherBody.isMonster &&
      reducedImpactMagnitude > impactThreshold &&
      impactFromAbove
    ) {
